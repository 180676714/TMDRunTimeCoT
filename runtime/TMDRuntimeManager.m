...
//  TFRuntimeDemo
//
//  Created by XXX on 17/6/16.
//  Copyright © 2017年 TechFantasy. All rights reserved.
//

#import "TMDRuntimeManager.h"

#import <objc/message.h>

@implementation TMDRuntimeManager
...
- (void)TF_archiveAndUnarchive:(Class)modelClass model:(id)model {
   archiveModelClass = modelClass;
    archiveModel = model;
    [TMDRuntimeManager TF_replaceMethodSourceClass:modelClass sourceSel:@selector(initWithCoder:) targetClass:[TMDRuntimeManager class] targetSel:@selector(initWithCoder:)];
    [TMDRuntimeManager TF_replaceMethodSourceClass:modelClass sourceSel:@selector(encodeWithCoder:) targetClass:[TMDRuntimeManager class] targetSel:@selector(encodeWithCoder:)];
}
- (instancetype)initWithCoder:(NSCoder *)coder
{
    ...
    if (archiveModel) {
        NSArray *ivarNames = [TMDRuntimeManager TF_getAllIvarWithClass:archiveModelClass];
